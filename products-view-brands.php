...
if (isset($_GET['brand_id'])) {
    ...
    $brandQuery = mysqli_query($dbcon, "SELECT * FROM sm_brands WHERE brand_id = '$brand_id'") or die(mysqli_error($dbcon));
    if ($brandRow = mysqli_fetch_assoc($brandQuery)) {
        $brand_name = $brandRow['brand_name'];
    }
?>
    <section class="bg-pri">
        <div class="container py-4">
            <h2 class="text-white font-bold text-center text-2xl lg:text-3xl"><?php echo $brand_name ?></h2>
        </div>
    </section>
    ...
    <section>
        <div class="container py-16">
            <?php
            $products = mysqli_query($dbcon, "SELECT * FROM sm_products WHERE brand_id = '$brand_id' AND pro_status != 0 ORDER BY pro_id DESC") or die(mysqli_error($dbcon));
